...
public class DocumentBatchFacadeService {
    ...
    public void completeBatch(DocumentBatch b, List<Map<String,Object>> results){
        ...
        for(var r:results){ if("OK".equals(r.get("status"))){ok++; String rs=String.valueOf(r.getOrDefault("resultStatus","")); if("APTO".equals(rs)) apt++; if("NO_APTO".equals(rs)) noApt++; pending++;} else err++; }
        ...
        b.setStatus(err>0?"PARTIAL_ERROR":"PENDING_REVIEW"); batchRepository.save(b);
    }
